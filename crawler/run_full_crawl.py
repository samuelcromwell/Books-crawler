...
async def parse_listing_for_book_links(html, base_url):
    from selectolax.parser import HTMLParser
    t = HTMLParser(html)
    for a in t.css("article.product_pod h3 a"):
        href = a.attributes["href"]
        yield urljoin(base_url, href.replace("../../../", "catalogue/"))

async def upsert_book(db, data):
    now = __import__("datetime").datetime.utcnow()
    data["content_hash"] = make_fingerprint(data)
    data["updated_at"] = now
    data.setdefault("created_at", now)
    data["crawl_meta"] = {"ts": now, "status": "ok", "source_url": data["product_url"]}

    existing = await db.books.find_one({"product_url": data["product_url"]}, {"content_hash":1})
    if not existing:
        await db.books.insert_one(data)
        await db.changes.insert_one({"product_url": data["product_url"], "change_type":"create",
                                     "changed_fields": {}, "ts": now})
        return "created"
    elif existing.get("content_hash") != data["content_hash"]:
        # compute changed fields (simple diff)
        old_doc = await db.books.find_one({"product_url": data["product_url"]})
        changed = {k: {"old": old_doc.get(k), "new": data.get(k)}
                   for k in data.keys() if old_doc.get(k) != data.get(k)}
        await db.books.replace_one({"_id": old_doc["_id"]}, data)
        await db.changes.insert_one({"product_url": data["product_url"], "change_type":"update",
                                     "changed_fields": changed, "ts": now})
        return "updated"
    return "skipped"
...
async def crawl_all():
    ...
    async with get_client() as client:
        ...
        t = HTMLParser(home)
        cat_links = [urljoin(base, a.attributes["href"])
